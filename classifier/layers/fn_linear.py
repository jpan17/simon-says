...
def fn_linear(input, params, hyper_params, backprop, dv_output=None):
    ...
    if num_in != hyper_params['num_in']:
        ...
        print('Incorrect number of inputs provided at linear layer.\n Got  inputs {0},  expected {1}.'.format(num_in, hyper_params['num_in']))
        raise
    ...
    if backprop:
        ...
        dv_input = np.dot(dv_output.T, W)
        dv_input = dv_input.T
        
        grad1 = np.dot(input, dv_output.T) / batch_size
        grad['W'] = grad1.T
    
        grad2 = np.sum(dv_output, axis = 1) / batch_size
        grad['b'] = np.reshape(grad2, (b.shape[0], b.shape[1]))
